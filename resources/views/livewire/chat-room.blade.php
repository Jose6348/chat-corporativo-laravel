<div class="h-full flex flex-col" id="chat-room">
    <!-- Header do Chat -->
    <div class="bg-gradient-to-r from-purple-600 via-blue-600 to-cyan-500 px-6 py-4 flex justify-between items-center border-b border-gray-700">
        <div class="flex items-center space-x-3">
            <a href="{{ route('dashboard') }}" class="text-white/80 hover:text-white transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                </svg>
            </a>
            <div class="bg-white/20 p-2 rounded-lg backdrop-blur-sm">
                <img src="{{ asset('logo-synkro.png') }}" alt="Synkro" class="w-6 h-6 object-contain filter brightness-0 invert">
            </div>
            <div>
        ...
        <div class="flex items-center space-x-2">
            <button 
                ...
                class="bg-red-600/20 hover:bg-red-600/30 text-red-300 hover:text-red-200 text-sm font-semibold py-2 px-4 rounded-lg border border-red-500/30 transition-all duration-200 flex items-center space-x-2">
            ...
            <form action="{{ route('channels.destroy', $channel) }}" 
                  method="POST" 
                  onsubmit="return confirm('Tem certeza que deseja excluir este canal? Todas as mensagens serão perdidas permanentemente.');"
                  class="inline">
                @csrf
                @method('DELETE')
                <button type="submit" 
    ...
    <!-- Área de Mensagens -->
    <div id="messages" class="flex-1 bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 p-6 overflow-y-auto border-b border-gray-700">
        ...
        <div class="mb-4 group"
            ...
            @if(($message['user']['id'] ?? null) === auth()->id())
                ...
                <div class="flex justify-end">
                    <div class="max-w-[70%] flex items-end space-x-2">
                        ...
                        <div class="bg-gradient-to-br from-cyan-500 to-blue-600 text-white rounded-2xl rounded-br-md px-4 py-3 shadow-lg group-hover:shadow-xl transition-all duration-200 relative">
                            <div class="flex items-center justify-between mb-1">
                                <span class="text-xs font-semibold text-cyan-100">Você</span>
                                @if(($message['user']['id'] ?? null) === auth()->id())
                                <button 
                            ...
                            <p class="text-sm leading-relaxed text-white">{{ $message['body'] ?? '' }}</p>
                            <span class="text-xs text-cyan-100 block mt-2 opacity-80">
                                {{ \Carbon\Carbon::parse($message['created_at'] ?? now())->format('H:i') }}
                            </span>
                        </div>
                        <div class="w-8 h-8 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-full flex items-center justify-center text-white font-semibold text-sm shadow-md ring-2 ring-gray-700">
            ...
            @else
                ...
                <div class="flex justify-start">
                    <div class="max-w-[70%] flex items-end space-x-2">
                        <div class="w-8 h-8 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full flex items-center justify-center text-white font-semibold text-sm shadow-md ring-2 ring-gray-700">
                            {{ strtoupper(substr($message['user']['name'] ?? 'U', 0, 1)) }}
                        </div>
                        <div class="bg-gray-700 border border-gray-600 rounded-2xl rounded-bl-md px-4 py-3 shadow-lg group-hover:shadow-xl group-hover:border-gray-500 transition-all duration-200">
                            <div class="flex items-center justify-between mb-1">
                                <span class="text-xs font-semibold text-gray-300">{{ $message['user']['name'] ?? 'Usuário' }}</span>
                            </div>
                            <p class="text-sm text-gray-100 leading-relaxed">{{ $message['body'] ?? '' }}</p>
                            <span class="text-xs text-gray-400 block mt-2 opacity-80">
                                {{ \Carbon\Carbon::parse($message['created_at'] ?? now())->format('H:i') }}
                            </span>
                        </div>
                        <div class="flex-1"></div>
        ...
        <div class="flex flex-col items-center justify-center h-full text-center py-12">
            <div class="bg-gradient-to-br from-purple-500/20 to-cyan-500/20 rounded-full p-8 w-24 h-24 mb-4 flex items-center justify-center backdrop-blur-sm border border-gray-700">
                <img src="{{ asset('logo-synkro.png') }}" alt="Synkro" class="w-16 h-16 object-contain opacity-50 filter brightness-0 invert">
            </div>
            <h3 class="text-lg font-semibold text-white mb-2">Nenhuma mensagem ainda</h3>
    ...
    <!-- Formulário de Envio -->
    <form wire:submit.prevent="sendMessage" class="bg-gray-800 border-t border-gray-700 p-4">
        <div class="flex space-x-3">
            <div class="flex-1 relative">
                <input 
                    ...
                    class="w-full px-4 py-3 pr-12 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all"
                    autocomplete="off">
            ...
            <button 
                type="submit" 
                class="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 text-white font-semibold py-3 px-6 rounded-lg transition-all duration-200 flex items-center space-x-2 shadow-lg hover:shadow-xl transform hover:scale-105 active:scale-95">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
